...
namespace Structs_Final_Project
{
    public class SDES
    {
        public int[] P10position = { 7, 0, 9, 6, 3, 5, 2, 4, 8, 1};
        public int[] P8position = { 6,4,9,8,1,3,0,5};
        public int[] P4position = { 2,0,1,3};
        public int[] EPposition = { 1,0,3,2,0,2,3,1};
        public int[] IPposition = { 2,7,0,5,1,6,3,4};
        public string k1,k2;
        ...
        public void KeysGenerator(string principalKey)
        {
            if (IsBinary(principalKey))
            {
                ...
                string P10 = "";
                for (int i = 0; i < P10position.Length; i++)
                {
                    P10 += newPrincipalKey[P10position[i]].ToString();
                }

                string subdiv1 = P10.Substring(0, 5);
                string subdiv2 = P10.Substring(5, 5);
                ...
                for (int i = 0; i < P8position.Length; i++)
                {
                    k1 += LS1[P8position[i]].ToString();
                }
                string LS2 = LS1.Substring(2, 8) + LS1[0].ToString() + LS1[1].ToString();
                
                for (int i = 0; i < P8position.Length; i++)
                {
                    k2 += LS2[P8position[i]].ToString();
                }
            }
        }
